...
{
	public static void drawImage(BufferedImage image,Vector<CoordDiff> points,Graphics2D graphics2d)
	{
		for (int i = 0; i < points.size(); i++)
		{
			int x= (int)( points.get(i).x);
			int y = (int)( points.get(i).y);
			int x2 = x + (int)(points.get(i).dx);
			int y2 = y + (int)(points.get(i).dy);
//			int x= (int)( points.get(i).x+ points.get(i).dx);
//			int y = (int)( points.get(i).x+ points.get(i).dy);
